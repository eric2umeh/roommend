    ...
    },
    {
      ...
      render: (value) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          ...
          value === 'NGO' ? 'bg-green-100 text-green-800' :
    ...
    },
    {
      ...
      render: (value) => (
        <span className={`px-2 py-1 rounded text-xs font-medium ${
          value === 'active' ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-800'
        }`}>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-2">Manage organizations and their hosted events</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg border border-slate-200 p-4">
          ...
          <div className="text-3xl font-bold text-slate-900">{mockOrganizations.length}</div>
        </div>
        <div className="bg-white rounded-lg border border-slate-200 p-4">
          <div className="text-sm text-slate-600 mb-1">Government</div>
          <div className="text-3xl font-bold text-blue-600">{mockOrganizations.filter(o => o.type === 'Government').length}</div>
        </div>
        <div className="bg-white rounded-lg border border-slate-200 p-4">
          <div className="text-sm text-slate-600 mb-1">NGOs</div>
          <div className="text-3xl font-bold text-green-600">{mockOrganizations.filter(o => o.type === 'NGO').length}</div>
        </div>
        <div className="bg-white rounded-lg border border-slate-200 p-4">
          <div className="text-sm text-slate-600 mb-1">Corporate</div>
          <div className="text-3xl font-bold text-purple-600">{mockOrganizations.filter(o => o.type === 'Corporate').length}</div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-lg shadow-lg max-w-lg w-full max-h-96 overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-slate-900 mt-1">{selectedOrg.contact_person}</p>
              </div>
              ...
              <div>
                <label className="text-sm font-medium text-slate-700">Total Rooms Booked</label>
                <p className="text-slate-900 mt-1 font-semibold">{selectedOrg.total_rooms} rooms</p>
              </div>

              <div>
                <label className="text-sm font-medium text-slate-700">Documents</label>
                <p className="text-slate-900 mt-1">{selectedOrg.documents_count} files uploaded</p>
              </div>
