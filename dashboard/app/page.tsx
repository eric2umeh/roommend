...
export default function DashboardPage() {
  ...
  const canViewFinancials =
    isAdmin || isManager || isAccounting || hasPermission("view_reports");
  const canViewReservations =
    isAdmin || isManager || isFrontDesk || hasPermission("manage_reservations");
  const canViewHousekeeping =
    isAdmin || isManager || isHousekeeping || hasPermission("manage_tasks");
  const canViewRestaurant =
    isAdmin || isManager || isKitchen || hasPermission("manage_orders");
  const canViewInventory =
    isAdmin || isManager || hasPermission("manage_inventory");
  const canViewStaff =
    isAdmin || isManager || isHR || hasPermission("manage_staff");
  const canViewOrganizations =
    isAdmin || isMarketing || hasPermission("manage_organizations");
  ...
  const occupancyRate = Math.round(
    (roomStats.occupied / roomStats.total) * 100,
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {canViewFinancials && (
            <div className="bg-white rounded-lg border border-slate-200 p-6 hover:shadow-md transition">
              ...
              <p className="text-xs text-slate-500 mt-2">
                From {mockReservations.length} reservations
              </p>
            </div>
          )}
            ...
            {mockReservations.map((res) => (
              ...
              >
                ...
                <div className="text-right">
                  <div className="font-medium text-slate-900">
                    ₦{res.total_price_naira.toLocaleString()}
                  </div>
                  <span
